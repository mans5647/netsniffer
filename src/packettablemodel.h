...
public:
    using ListType = QList<FrameInfo>;

    PacketTableModel(const PacketTableModel &);
    PacketTableModel();
    ...
    void append(const FrameInfo &);
    const FrameInfo & get(int) const;
    const FrameInfo & operator[](int) const;
    FrameInfo& operator[](int);
    void clear() noexcept;


    ListType * get_list_ptr();
